...
\end{problem}
\begin{solution}
This is a weaker case of the banach fixed point theorem. We will prove that the sequence $\{a_n\}$ is cauchy which will imply that its convergent in $\bR$, with limit being a fixed point as
...
because $c^{N-2}>c^{n-2}$ for $c\in(0,1)$. 
Now, given any $\varepsilon>0$, as the right hand side clearly tends to $0$ as $N$ tends to infinity, we can find some $M>0$ such that it is less than $\varepsilon$ and by the previous inequality, for all $m,n>M$ we must have,
$$|a_m-a_n|<\varepsilon$$
