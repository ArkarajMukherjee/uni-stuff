...
\end{problem}
\begin{solution}
  Firstly take $X,Y$ such that for all $x\in A$ we have $|x|\leq X$ and for all $y\in Y$ we have $|y|\leq Y$, these constants exist by the assumption that $A,B$ are bounded in the question. Below we first prove boundedness and then prove or disprove the claims.\newline
  (a) The claim is true. For any $x\in A\cup B$ we see that $x$ is either in $A$ or in $B$ and if $x\in A$ then $|x|\leq X\leq\max\{X,Y\}$ and if $x\in B$ then $|x|\leq Y\leq\max\{X,Y\}$ thus for all $x\in A\cup B$ we have that $|x|\leq\max\{X,Y\}$ so these are bounded and a supremum exists. Let $m=\sup(A\cup B)$, then for all $x\in A,B$ we have that $x\leq m$ thus $m\geq\sup A,\sup B$ which implies that $m\geq \max\{\sup A,\sup B\}.$ Now if $x\in A\cup B$ then $x\in A$ or $x\in B$ and if $x\in A$ then $x\leq \sup A\leq m$ and if $x\in B$ then $x\leq\sup B\leq m$ thus we see that $\sup(A\cup B)\leq m$. This implies that $\sup(A\cup B)=\max\{\sup A,\sup B\}$. \newline 
  (b) The claim is false. For any $x\in A\cap B$ we see that $x\in A$ so $|x|\leq X$ and thus its bounded. Now consider $A=\{0,2\}$ and $B=\{0,1\}$, then we have that $\min\{\sup A,\sup B\}=\min\{1,2\}=1\neq 0=\sup\{0\}=\sup A\cap B$. \newline
  (c) This claim is true. For any $x+y\in A+B$ with $x\in A$ and $y\in B$, by the triangle inequality we have $|x+y|\leq|x|+|y|\leq X+Y$ as $|x|\leq X$ and $|y|\leq Y$ so this is bounded. Say $a=\sup A$ and $b=\sup B$ for brevity. Then for all $x+y\in A+B$ with $x\in A,y\in B$ we see that $x\leq a$ and $y\leq b$ which implies that $x+y\leq a+b$ thus $a+b$ is an upper bound for $A+B.$ Now given any $\varepsilon>0$ we can find $x_o+y_o\in[a+b-\varepsilon,a+b]$ by choosing $x_o$ in $A$ and $y_o$ in $B$ such that $x_o\in[a-\varepsilon/2,a]$ and $y_o\in[b-\varepsilon/2,b]$ (which exist as we defined $a,b$ to be the supremums of $A,B$ respectively) as $$a+b-\varepsilon=(a-\varepsilon/2)+(b-\varepsilon/2)\leq x_o+y_o\leq a+b$$This is enough to conclude that $a+b$ is the supremum of $A+B$. (It was shown in class that if for some nonempty set $S\subseteq\bR,s$ is an upper bound such that for all $\varepsilon>0$ we have $S\cap[s-\varepsilon,s]\neq\emptyset$ then its the supremum.) \newline
  (d) This is false. For any $x-y\in A-B$ with $x\in A$ and $y\in B$ we can use the triangle inequality to get $|x-y|\leq |x|+|-y|=|x|+|y|\leq X+Y$ as $|x|\leq X$ and $|y|\leq Y$. Consider $A=\{0\}$ and $B=[-1,1]$. Then we have that $A-B=\{0-x:x\in[-1,1]\}=[-1,1]$ and thus $\sup(A-B)=1$. But as $\sup A=0$ and $\sup B=1$, $\sup(A-B)=1$ is not equal to $\sup A-\sup B=0-1=-1$ disproving the statement. \newline
  (e) This is false. For $xy\in AB$ with $x\in A$ and $y\in B$ we have that $|x|\leq X$ and as $|y|\geq 0$ by the monotonicity of multiplication we have $|x||y|\leq X|y| \leq XY$ as $|y|\leq Y$ thus we see that $|xy|=|x||y|\leq XY$ so its bounded. Consider $A=\{-1\}$ and $B=[-1,1]$. We see that $AB=\{-x:x\in[-1,1]\}=[-1,1]$ and thus $\sup AB=1$ but this is not equal to $(\sup A)(\sup B)=(-1)(1)=-1$ disproving the statement.
\end{solution}
