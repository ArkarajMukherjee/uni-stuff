...
Thus this holds for all $m\geq n$ and taking the limits as $m\to\infty$ on all of the terms in this inequality we get, 
	...
	which is enough to conclude that $a_n/b_n\longrightarrow v$ as well. For an example consider $a_n=\sin(n)/n-1$ and $b_n=1/n^2$, both of these tend to $0$ and $b_n$ decreases monotonically. Now using taylor series we can easily show that, $a_{n+1}-a_n=\frac{-1}{6n^3}+O(n^{-5})$ hence, 
	$$\frac{a_{n+1}-a_n}{b_{n+1}-b_n}\xrightarrow{n\to\infty}\frac{-1}{6}$$
	Thus using the result we see that,
	$$\frac{a_n}{b_n}\xrightarrow{n\to\infty}\frac{-1}{6}$$
\end{solution}
