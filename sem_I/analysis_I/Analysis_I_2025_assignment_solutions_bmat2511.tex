...
The first factor is rational, being a finite sum of rationals and the second factor is a geometric series converging to $1/(1-2^{-p})$ so the whole thing is rational, being a product of two rationals. As the periodic and nonperiodic parts are both rational, the original number being a sum of those two is also rational. 
	For the other direction, let $x = a/b$ be a rational number in $[0,1]$, with $a, b \in \bZ$, $b \neq 0$. The bits $b_k$ of its binary expansion are generated by the algorithm: $r_0 = x$, and for $k \ge 1$,
  $$ b_k = \lfloor 2r_{k-1} \rfloor, \quad r_k = 2r_{k-1} - b_k $$
  We can show that each remainder $r_k$ is a rational number with denominator $b$. We have $r_1 = 2(a/b) - b_1 = \frac{2a-b_1b}{b}$. Let $a_1 = 2a-b_1b$. Inductively, if $r_{k-1} = a_{k-1}/b$ for some integer $a_{k-1}$, then $r_k = 2(a_{k-1}/b) - b_k = \frac{2a_{k-1}-b_kb}{b}$. The new numerator $a_k = 2a_{k-1}-b_kb$ is also an integer.
  
  By the construction of the algorithm, we always have $0 \le r_k < 1$. This implies $0 \le a_k/b < 1$, which means the integer numerator $a_k$ must belong to the finite set $\{0, 1, \ldots, b-1\}$.
  
  Since the sequence of numerators $(a_1, a_2, a_3, \ldots)$ can only take on a finite number of values, the Pigeonhole Principle dictates that a value must eventually repeat. Let $M$ and $p$ be the smallest positive integers such that $a_M = a_{M+p}$. This equality of numerators implies an equality of remainders: $r_M = a_M/b = a_{M+p}/b = r_{M+p}$.
  
  This repetition forces the entire subsequent process to cycle. We have:
  \begin{itemize}
    \item $b_{M+1} = \lfloor 2r_M \rfloor = \lfloor 2r_{M+p} \rfloor = b_{M+p+1}$
    \item $r_{M+1} = 2r_M - b_{M+1} = 2r_{M+p} - b_{M+p+1} = r_{M+p+1}$
  \end{itemize}
  By induction, it follows that $b_k = b_{k+p}$ for all $k \ge M$, which is the definition of a periodic binary expansion. If some $a_k=0$, the expansion terminates with zeros, which is a special case of a periodic sequence.

  Using this we can see that the set of rational numbers in $[0,1]$, which we denote $\bQ \cap [0,1]$, is precisely the set of numbers in that interval which have a periodic binary expansion.
  
  Let $S$ be the set of all periodic binary expansions. There is a surjective map from $S$ to $\bQ \cap [0,1]$. To prove that $\bQ \cap [0,1]$ is countable, it suffices to show that $S$ is countable.
...
Both the sequences in $2$ and $3$ converge to $0$ as exponential growth dominates polynomial growth as shown in class and $n^2$ dominates $n$ as well. As these are convergent sequences, they have the same limit extremums and limits which is $0$.
\end{solution}
...
Where we used this fact proven in class: For bounded sequences of real numbers $\{a_n\}_{n\in\bN}$ we have, $$\limsup_{n\to\infty}a_n=-\liminf_{n\to\infty}(-a_n)$$Here equality might not occur in either case, for example when $a_n=(-1)^n$ and $b_n=-a_n$, in this case we see that
