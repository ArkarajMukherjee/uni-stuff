...
\end{kframe}
\end{knitrout}
	The test sh=hows us a p-value of approximately $0.2623$. Since the p-value is greater than standard significance levels (like $0.05$), we fail to reject the null hypothesis. The researchers claim is indeed plausible as the observed sample does not significantly deviate from what we would expect under random, proportional sampling.	
...
\end{kframe}
\end{knitrout}
	The computed test statistic $W$ (which matches our calculated $U_X$) is $89.5$. The p-value is approximately $0.3259$. Because the p-value is greater than 0.05, we fail to reject the null hypothesis. We do not have sufficient statistical evidence to conclude that the two populations are not equal in distribution.
